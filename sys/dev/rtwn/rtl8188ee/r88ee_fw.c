...
#include <dev/rtwn/rtl8188ee/r88ee_fw_cmd.h>
#include <dev/rtwn/rtl8188ee/r88ee_tx_desc.h>


#ifndef RTWN_WITHOUT_UCODE
static int
r88ee_fw_cmd(struct rtwn_softc *sc, uint8_t id, const void *buf, int len)
{
#if 0
	struct r88ee_fw_cmd cmd;
	int ntries, error;
...
#ifndef RTWN_WITHOUT_UCODE
static int
r88ee_send_ra_cmd(struct rtwn_softc *sc, int macid, uint32_t rates,
    int maxrate)
{
#if 0
	struct r88ee_fw_cmd_macid_cfg cmd;
	uint8_t mode;
	int error = 0;
...
static void
r88ee_init_ra(struct rtwn_softc *sc, int macid)
{
#if 0
	...
	if (sc->sc_ratectl == RTWN_RATECTL_FW) {
		r88ee_send_ra_cmd(sc, macid, rates, maxrate);
	}
#endif
...
void
r88ee_joinbss_rpt(struct rtwn_softc *sc, int macid)
{
#if 0
#ifndef RTWN_WITHOUT_UCODE
	struct r88ee_softc *rs = sc->sc_priv;
	struct ieee80211vap *vap;
	struct r88ee_fw_cmd_joinbss_rpt cmd;
...
#ifndef RTWN_WITHOUT_UCODE
int
r88ee_set_rsvd_page(struct rtwn_softc *sc, int probe_resp, int null,
    int qos_null)
{
#if 0
	struct r88ee_fw_cmd_rsvdpage rsvd;
...
int
r88ee_set_pwrmode(struct rtwn_softc *sc, struct ieee80211vap *vap,
    int off)
{
#if 0
	struct r88ee_fw_cmd_pwrmode mode;
	int error;
...
void
r88ee_set_rssi(struct rtwn_softc *sc)
{
#if 0
	struct ieee80211_node *ni;
	struct rtwn_node *rn;
	struct r88ee_fw_cmd_rssi cmd;
	int i;
...
static void
r88ee_ratectl_tx_complete(struct rtwn_softc *sc, uint8_t *buf, int len)
{
#if 0
#if __FreeBSD_version >= 1200012
	struct ieee80211_ratectl_tx_status txs;
#endif
	struct r88ee_c2h_tx_rpt *rpt;
	struct ieee80211_node *ni;
	uint8_t macid;
	int ntries;
	...
	rpt = (struct r88ee_c2h_tx_rpt *)buf;
	if (len != sizeof(*rpt)) {
		device_printf(sc->sc_dev,
...
void
r88ee_handle_c2h_report(void *arg)
{
#if 0
