...
/* XXX 92CU? */
static int
r88ee_iq_calib_chain(struct rtwn_softc *sc, int chain, uint16_t tx[2],
    uint16_t rx[2])
{
#if 0
	uint32_t status;
...
static void
r88ee_iq_calib_run(struct rtwn_softc *sc, int n, uint16_t tx[2][2],
    uint16_t rx[2][2], struct r88ee_iq_cal_reg_vals *vals)
{
#if 0
	/* Registers to save and restore during IQ calibration. */
	...
	if (n == 0) {
		...
		vals->txpause = rtwn_read_1(sc, R88EE_TXPAUSE);
		vals->bcn_ctrl[0] = rtwn_read_1(sc, R88EE_BCN_CTRL(0));
		vals->bcn_ctrl[1] = rtwn_read_1(sc, R88EE_BCN_CTRL(1));
		vals->gpio_muxcfg = rtwn_read_4(sc, R88EE_GPIO_MUXCFG);
	}
	...
	hssi_param1 = rtwn_bb_read(sc, R88EE_HSSI_PARAM1(0));
	if (!(hssi_param1 & R88EE_HSSI_PARAM1_PI)) {
		rtwn_bb_write(sc, R88EE_HSSI_PARAM1(0),
		    hssi_param1 | R88EE_HSSI_PARAM1_PI);
		rtwn_bb_write(sc, R88EE_HSSI_PARAM1(1),
		    hssi_param1 | R88EE_HSSI_PARAM1_PI);
	}

	if (n == 0) {
		vals->ofdm0_trxpathena =
		    rtwn_bb_read(sc, R88EE_OFDM0_TRXPATHENA);
		vals->ofdm0_trmuxpar = rtwn_bb_read(sc, R88EE_OFDM0_TRMUXPAR);
		vals->fpga0_rfifacesw1 =
		    rtwn_bb_read(sc, R88EE_FPGA0_RFIFACESW(1));
	}

	rtwn_bb_write(sc, R88EE_OFDM0_TRXPATHENA, 0x03a05600);
	rtwn_bb_write(sc, R88EE_OFDM0_TRMUXPAR, 0x000800e4);
	rtwn_bb_write(sc, R88EE_FPGA0_RFIFACESW(1), 0x22204000);
	if (sc->ntxchains > 1) {
		rtwn_bb_write(sc, R88EE_LSSI_PARAM(0), 0x00010000);
		rtwn_bb_write(sc, R88EE_LSSI_PARAM(1), 0x00010000);
	}
	...
	rtwn_write_1(sc, R88EE_BCN_CTRL(1),
	    vals->bcn_ctrl[1] & ~R88EE_BCN_CTRL_EN_BCN);
	rtwn_write_1(sc, R88EE_GPIO_MUXCFG,
	    vals->gpio_muxcfg & ~R88EE_GPIO_MUXCFG_ENBT);
	...
	for (chain = 0; chain < sc->ntxchains; chain++) {
		...
		for (i = 0; i < 2; i++) {
			...
			ret = r88ee_iq_calib_chain(sc, chain,
			    tx[chain], rx[chain]);
			if (ret == 0) {
	...
	rtwn_bb_write(sc, R88EE_OFDM0_TRXPATHENA,
	    vals->ofdm0_trxpathena);
	rtwn_bb_write(sc, R88EE_FPGA0_RFIFACESW(1),
	    vals->fpga0_rfifacesw1);
	rtwn_bb_write(sc, R88EE_OFDM0_TRMUXPAR, vals->ofdm0_trmuxpar);

	rtwn_bb_write(sc, 0x0e28, 0x00);
	rtwn_bb_write(sc, R88EE_LSSI_PARAM(0), 0x00032ed3);
	if (sc->ntxchains > 1)
		rtwn_bb_write(sc, R88EE_LSSI_PARAM(1), 0x00032ed3);

	if (n != 0) {
		if (!(hssi_param1 & R88EE_HSSI_PARAM1_PI)) {
			rtwn_bb_write(sc, R88EE_HSSI_PARAM1(0), hssi_param1);
			rtwn_bb_write(sc, R88EE_HSSI_PARAM1(1), hssi_param1);
		}

		for (i = 0; i < nitems(reg_adda); i++)
			rtwn_bb_write(sc, reg_adda[i], vals->adda[i]);

		rtwn_write_1(sc, R88EE_TXPAUSE, vals->txpause);
		rtwn_write_1(sc, R88EE_BCN_CTRL(0), vals->bcn_ctrl[0]);
		rtwn_write_1(sc, R88EE_BCN_CTRL(1), vals->bcn_ctrl[1]);
...
static int
r88ee_iq_calib_compare_results(struct rtwn_softc *sc, uint16_t tx1[2][2],
...
static void
r88ee_iq_calib_write_results(struct rtwn_softc *sc, uint16_t tx[2],
    uint16_t rx[2], int chain)
{
	...
	reg = rtwn_bb_read(sc, R88EE_OFDM0_TXIQIMBALANCE(chain));
	val = ((reg >> 22) & 0x3ff);
	x = tx[0];
	if (x & 0x00000200)
		x |= 0xfffffc00;
	reg = (((x * val) >> 8) & 0x3ff);
	rtwn_bb_setbits(sc, R88EE_OFDM0_TXIQIMBALANCE(chain), 0x3ff, reg);
	rtwn_bb_setbits(sc, R88EE_OFDM0_ECCATHRESHOLD, 0x80000000,
	    ((x * val) & 0x80) << 24);
	...
	if (y & 0x00000200)
		y |= 0xfffffc00;
	tx_c = (y * val) >> 8;
	rtwn_bb_setbits(sc, R88EE_OFDM0_TXAFE(chain), 0xf0000000,
	    (tx_c & 0x3c0) << 22);
	rtwn_bb_setbits(sc, R88EE_OFDM0_TXIQIMBALANCE(chain), 0x003f0000,
	    (tx_c & 0x3f) << 16);
	rtwn_bb_setbits(sc, R88EE_OFDM0_ECCATHRESHOLD, 0x20000000,
	    ((y * val) & 0x80) << 22);

	if (rx[0] == 0xff || rx[1] == 0xff)
		return;

	rtwn_bb_setbits(sc, R88EE_OFDM0_RXIQIMBALANCE(chain), 0x3ff,
	    rx[0] & 0x3ff);
	rtwn_bb_setbits(sc, R88EE_OFDM0_RXIQIMBALANCE(chain), 0xfc00,
	    (rx[1] & 0x3f) << 10);

	if (chain == 0) {
		rtwn_bb_setbits(sc, R88EE_OFDM0_RXIQEXTANTA, 0xf0000000,
		    (rx[1] & 0x3c0) << 22);
	} else {
		rtwn_bb_setbits(sc, R88EE_OFDM0_AGCRSSITABLE, 0xf000,
		    (rx[1] & 0x3c0) << 6);
	}
#else
	printf("RTL8188EE:%s not implemented\n", __func__);
...
r88eee_iq_calib(struct rtwn_softc *sc)
{
	...
	for (n = 0; n < RTWN_IQ_CAL_NRUN; n++) {
		r88ee_iq_calib_run(sc, n, tx[n], rx[n], &vals);

		if (n == 0)
			continue;

		/* Valid results remain stable after consecutive runs. */
		valid = r88ee_iq_calib_compare_results(sc, tx[n - 1],
		    rx[n - 1], tx[n], rx[n]);
		if (valid)
			break;
	}
	...
	if (valid) {
		r88ee_iq_calib_write_results(sc, tx[n][0], rx[n][0], 0);
		if (sc->ntxchains > 1)
