...
/* XXX 92CU? */
static int
r88eee_iq_calib_chain(struct rtwn_softc *sc, int chain, uint16_t tx[2],
    uint16_t rx[2])
{
#if 0
	uint32_t status;
...
static void
r88eee_iq_calib_run(struct rtwn_softc *sc, int n, uint16_t tx[2][2],
    uint16_t rx[2][2], struct r88eee_iq_cal_reg_vals *vals)
{
#if 0
	/* Registers to save and restore during IQ calibration. */
	...
	for (chain = 0; chain < sc->ntxchains; chain++) {
		...
		for (i = 0; i < 2; i++) {
			...
			ret = r88eee_iq_calib_chain(sc, chain,
			    tx[chain], rx[chain]);
			if (ret == 0) {
...
static int
r88eee_iq_calib_compare_results(struct rtwn_softc *sc, uint16_t tx1[2][2],
...
static void
r88eee_iq_calib_write_results(struct rtwn_softc *sc, uint16_t tx[2],
...
r88eee_iq_calib(struct rtwn_softc *sc)
{
	...
	if (valid) {
		r88eee_iq_calib_write_results(sc, tx[n][0], rx[n][0], 0);
		if (sc->ntxchains > 1)
			r88eee_iq_calib_write_results(sc, tx[n][1], rx[n][1], 1);
	}
#else
	device_printf(sc->sc_dev, "Unimplemented\n");
#endif
